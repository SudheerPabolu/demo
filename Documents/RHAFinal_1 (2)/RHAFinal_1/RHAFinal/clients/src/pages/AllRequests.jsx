...
//       try {
//         const res = await axios.get('/api/admin/all-requests', { withCredentials: true });
//         setData(res.data);
//       } catch (err) {
...
//       {data ? (
//         <>
//           {Object.entries(data).map(([type, requests]) => (
//             <div key={type} className="request-section">
//               <h3>{type.charAt(0).toUpperCase() + type.slice(1)} Requests</h3>
//               {requests.length === 0 ? (
...
//                     {requests.map((req, i) => (
//                       <tr key={i}>
//                         <td>{req.name || req.studentName || 'N/A'}</td>
//                         <td>{req.status}</td>
//                         <td>{new Date(req.createdAt || req.date || req.timestamp).toLocaleDateString()}</td>
//                       </tr>
//                     ))}
//                   </tbody>
//                 </table>
